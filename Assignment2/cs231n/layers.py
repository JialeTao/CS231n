...
def affine_forward(x, w, b):
    ...
    xraw = x.reshape(input_num, np.prod(input_size))
    out = xraw.dot(w) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * x_norm, axis=0)
    dbeta = np.sum(dout, axis=0)
    dx = 1.0 / (N * np.sqrt(sample_var + eps)) * gamma \
         * (N * dout - np.sum(dout, axis=0)
            + np.sum(dout * x_norm, axis=0)
            * (1.0 / N * np.sum(x_norm, axis=0) - x_norm))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for j in range(out_h):
        for k in range(out_w):
            ...
            for i in range(F):
                out[:, i, j, k] = np.sum(x_pad[:, :, h_coord:h_coord + HH, w_coord:w_coord + WW] * w[i, :, :, :],
                                         axis=(1, 2, 3))
...
def conv_backward_naive(dout, cache):
    ...
    for j in range(out_h):
        for k in range(out_w):
            ...
            for i in range(F):
                dw[i, :, :, :] += np.sum(
                    x_pad[:, :, h_coord:h_coord + HH, w_coord:w_coord + WW] * (dout[:, i, j, k])[:, None, None, None],
                    axis=0)
